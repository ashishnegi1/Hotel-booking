...
exports.createHotelPost = async (req, res) => {
    try{
        ...
        await hotel.save();
        req.flash('success', `${hotel.hotel_name} was created successfully`);
...
exports.editRemoveGet = (req, res) => {
    res.render('edit_remove', {title: 'Search for hotel to edit or delete'});
}
...
exports.editRemovePost = async (req, res, next) => {
    try {
        ...
        if (hotelData.length > 0) {
            ...
            req.flash('info', 'No hotel was found...');
...
exports.deleteHotelPost = async (req, res, next) => {
    try {
        ...
        req.flash('success', `Hotel ID: ${hotelId} has been deleted`);
        res.redirect(`/`);
    } catch (error) {
        next(error);
    }
